...
public class ArrowSource2ArrowSource {
  ...
  public boolean evaluateClause5() {
    ...
    if (this.sourceArrowSource instanceof Circle && 
        (_markerForOutgoingArrows_tuple = trafo.relation_markerForOutgoingArrows((Circle)this.sourceArrowSource)) != null)
    {
      targetArrowSource =  _markerForOutgoingArrows_tuple.t;
      _markerForOutgoingArrows = true;
    }
    return _markerForOutgoingArrows;
  }
  
  public boolean execute(final Transformation transformation) {
    this.trafo = transformation;
    // source = sourceArrowSource : Shape {..}
    if (sourceArrow.getSource() == null) {trafo.logFailure(10);
    return false;
    }trafo.logSuccess(10);
    sourceArrowSource =  sourceArrow.getSource();
    // Source2Target(sourceArrow, targetArrow)
    if (!evaluateClause4()) {trafo.logFailure(20);
    return false;
    }trafo.logSuccess(20);
    // markerForOutgoingArrows(sourceArrowSource, targetArrowSource)
    if (!evaluateClause5()) {trafo.logFailure(21);
    return false;
    }trafo.logSuccess(21);

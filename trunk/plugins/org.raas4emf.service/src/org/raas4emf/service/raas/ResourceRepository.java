...
/**
 * Implements {@link IRepository} using a {@link Resource}. Thus, one can abstract from the repository technology (e.g. Teneo, CDO, text files). *
 */
abstract public class ResourceRepository implements IRepository {
	...
	protected void fixProxy(EObject proxy) {
		((BasicEObjectImpl) proxy).eSetProxyURI(res.getURI().appendFragment(res.getURIFragment(proxy)));
	}
}

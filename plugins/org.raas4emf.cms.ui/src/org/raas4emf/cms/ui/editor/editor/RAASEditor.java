...
/**
 * This is an example of a RAAS model editor. <!-- begin-user-doc --> <!-- end-user-doc -->
 * 
 * @generated
 */
public class RAASEditor extends FormEditor implements IEditingDomainProvider, ISelectionProvider, IMenuListener, IViewerProvider {
	 ...
	 * @generated
	 */
	public static final List<String> FILE_EXTENSION_FILTERS = prefixExtensions(Arrays.asList(RAASEditorPlugin.INSTANCE.getString("_UI_RAASEditorFilenameExtensions").split("\\s*,\\s*")), "*.");

	protected static final String EDITOR_ID = "org.raas4emf.cms.ui.RAASEditor";
		...
		public void partActivated(IWorkbenchPart p) {
			if (p instanceof ContentOutline) {
				if (((ContentOutline) p).getCurrentPage() == contentOutlinePage) {
					getActionBarContributor().setActiveEditor(RAASEditor.this);
	...
	protected void handleActivate() {
		...
		if (!removedResources.isEmpty()) {
			if (handleDirtyConflict()) {
				getSite().getPage().closeEditor(RAASEditor.this, false);
			} else {
	...
	protected void updateProblemIndication() {
		if (updateProblemIndication) {
			BasicDiagnostic diagnostic = new BasicDiagnostic(Diagnostic.OK, "org.raas4emf.cms.ui.editor", 0, null, new Object[] { editingDomain.getResourceSet() });
	...
	protected void initializeEditingDomain() {
		...
		commandStack.addCommandStackListener(new CommandStackListener() {
			public void commandStackChanged(final EventObject event) {
				...
				if (editorInput.getModelElement() instanceof Artifact) {
					Artifact artifact = (Artifact) editorInput.getModelElement();
					RAASUtils.setModificationDate(artifact, new Date());
				}
	...
	protected void createContextMenuFor(StructuredViewer viewer) {
		...
		int dndOperations = DND.DROP_LINK;
	...
	public Diagnostic analyzeResourceProblems(Resource resource, Exception exception) {
		if (!resource.getErrors().isEmpty() || !resource.getWarnings().isEmpty()) {
			BasicDiagnostic basicDiagnostic = new BasicDiagnostic(Diagnostic.ERROR, "org.raas4emf.cms.ui.editor", 0, getString("_UI_CreateModelError_message", resource.getURI()), new Object[] { exception == null ? (Object) resource : exception });
			basicDiagnostic.merge(EcoreUtil.computeDiagnostic(resource, true));
			return basicDiagnostic;
		} else if (exception != null) {
			return new BasicDiagnostic(Diagnostic.ERROR, "org.raas4emf.cms.ui.editor", 0, getString("_UI_CreateModelError_message", resource.getURI()), new Object[] { exception });
		} else {
	...
	public void createPages() {
		...
		if (!getEditingDomain().getResourceSet().getResources().isEmpty()) {
			...
			//
			{
				ViewerPane viewerPane = new ViewerPane(getSite().getPage(), RAASEditor.this) {
					@Override
	...
	public IPropertySheetPage getPropertySheetPage() {
		PropertySheetPage propertySheetPage = new ExtendedPropertySheetPage(editingDomain) {
			...
			public void setSelectionToViewer(List<?> selection) {
				RAASEditor.this.setSelectionToViewer(selection);
				RAASEditor.this.setFocus();
			}
	...
	private static String getString(String key) {
		return RAASEditorPlugin.INSTANCE.getString(key);
	}
	...
	private static String getString(String key, Object s1) {
		return RAASEditorPlugin.INSTANCE.getString(key, new Object[] { s1 });
	}

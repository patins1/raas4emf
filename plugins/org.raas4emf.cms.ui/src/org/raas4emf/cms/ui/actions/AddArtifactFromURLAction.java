	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		for (Object element : RAASUIUtils.getSelection(event)) {
			if (element instanceof Folder) {
				...
				execute(HandlerUtil.getActiveWorkbenchWindowChecked(event), root, shell, false, null);
			}
		}
		return null;
	}

	public void execute(IWorkbenchWindow window, final Folder root, final Shell shell, boolean unzipWithoutAsking, final Runnable finalizeAction) throws ExecutionException {
		...
		if (dialog.open() == Window.OK) {
			...
			final boolean unzip = url.toLowerCase().endsWith(".zip") && (unzipWithoutAsking || MessageDialog.openQuestion(shell, "Unzipping Contents", "Do you want to unzip the zipped file instead of uploading it as zip file?"));

			if (unzip) {
				try {
					window.getActivePage().showView("org.eclipse.ui.views.ProgressView", null, IWorkbenchPage.VIEW_ACTIVATE);

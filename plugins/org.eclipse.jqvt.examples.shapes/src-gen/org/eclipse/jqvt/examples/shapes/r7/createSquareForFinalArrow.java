...
public class createSquareForFinalArrow {
  private Transformation trafo;
  
  private int hash;
  
  public createSquareForFinalArrow(final Circle s) {
    this.s=s;
  }
  ...
  public boolean equals(final Object obj) {
    ...
    if (!(obj instanceof createSquareForFinalArrow)) return false;
    createSquareForFinalArrow other = (createSquareForFinalArrow) obj;
  ...
  public boolean execute(final Transformation transformation) {
    ...
    // s.outArrow.isEmpty()
    if (!evaluateClause3()) {trafo.logFailure(175);
    return false;
    }trafo.logSuccess(175);
    // block = sourceblock : Block {..}
    if (s.getBlock() == null) {trafo.logFailure(164);
    return false;
    }trafo.logSuccess(164);
    sourceblock =  s.getBlock();
    // Source2Target(sourceblock, targetblock)
    if (!evaluateClause5()) {trafo.logFailure(176);
    return false;

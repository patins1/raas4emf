...
def generate_objects(data, children = None):
    chunks = []
    childrenCount = 0
    ...
    for obj in children :
        ...
        objects, nobjects = generate_objects(data,obj.children)
        childrenCount += nobjects
        ...
        if obj.type == "MESH" and obj.THREE_exportGeometry:
            ...
            if data["flipyz"]:
                matrix_world = ROTATE_X_PI2 * obj.matrix_local
            else:
                matrix_world = obj.matrix_local
        ...
        elif obj.type == "EMPTY" or (obj.type == "MESH" and not obj.THREE_exportGeometry) or nobjects>0:
    ...
    return ",\n\n".join(chunks), len(chunks) + childrenCount
...
def export_scene(scene, filepath, flipyz, option_colors, option_lights, option_cameras, option_embed_meshes, embeds, option_url_base_html, option_copy_textures):
    ...
    scene_text = json.dumps( json.loads( scene_text ), separators = (',', ':') )
